 ...
 *
 * Data persistence:
 *  • Reads  → Netlify Blobs (updated data) → bundled Excel fallback
 *  • Writes → Netlify Blobs  (POST /upload, POST /fetch-from-url)
 */
...
/* ── Excel data resolution ───────────────────────────────────────────── */

const BUNDLED_EXCEL = path.join(
  process.cwd(),
  "netlify/functions/data/fuel-subsidy.xlsx",
);
...
async function getExcelBuffer(): Promise<Buffer> {
  try {
    ...
    // Blobs not configured — fall through to bundled file
  }

  if (fs.existsSync(BUNDLED_EXCEL)) {
    return fs.readFileSync(BUNDLED_EXCEL);
  }
  throw new Error(
    "No Excel data found. Please upload a fuel-subsidy.xlsx file via the dashboard.",
  );
}
...
function downloadUrl(
  ...
  return new Promise((resolve, reject) => {
    ...
    lib.get(
      { hostname: parsed.hostname, port: parsed.port || undefined, path: parsed.pathname + parsed.search, headers: { "User-Agent": "Mozilla/5.0", ...extraHeaders } },
      (res) => {
...
async function getSharePointToken(spUrl: string, username: string, password: string): Promise<string | null> {
  try {
    ...
    return new Promise((resolve) => {
      ...
      const opts = { hostname: "login.microsoftonline.com", path: `/${tenant}/oauth2/token`, method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded", "Content-Length": Buffer.byteLength(bodyStr), "User-Agent": "Mozilla/5.0" } };
...
async function handleProvinceOverview(): Promise<Response> {
  ...
  for (let i = 12; i <= 19; i++) {
    ...
    rows.push({ province, sentToSbp: safeNum(row[2]), returnedBySbp: safeNum(row[3]), balanceWithSbp: safeNum(row[4]), qtyProcessedBySbp: safeNum(row[5]), amountDisbursedBySbp: safeNum(row[6]), pendingWithSbp: safeNum(row[7]) });
  }
  return json(rows);
}
...
async function handleVehicleBreakdown(): Promise<Response> {
  ...
  for (let i = 23; i <= 28; i++) {
    ...
    rows.push({ vehicleType, sentToSbp: safeNum(row[2]), returnedBySbp: safeNum(row[3]), balanceWithSbp: safeNum(row[4]), qtyProcessedBySbp: safeNum(row[5]), amountDisbursedBySbp: safeNum(row[6]), pendingWithSbp: safeNum(row[7]) });
  }
  return json(rows);
}
...
async function handleUpload(req: Request): Promise<Response> {
  ...
  return json({ success: true, message: "Excel file uploaded and saved successfully." });
}

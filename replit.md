...
- **Data reading**: Function reads from Netlify Blobs first (updated data), falls back to the bundled `fuel-subsidy.xlsx`
- **Data writing**: File uploads and URL fetches write to Netlify Blobs for persistent storage
- **Excel bundling**: Build command copies `artifacts/api-server/data/fuel-subsidy.xlsx` → `netlify/functions/data/` and `included_files` bundles it with the function

### Netlify Build Command (auto-configured in netlify.toml)
```
pnpm install && pnpm --filter @workspace/api-spec run codegen && mkdir -p netlify/functions/data && cp artifacts/api-server/data/fuel-subsidy.xlsx netlify/functions/data/fuel-subsidy.xlsx && pnpm --filter @workspace/fuel-dashboard run build:netlify
```
